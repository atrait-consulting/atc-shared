...
async function avecDelai<T>(travail: Promise<T>, secours: T): Promise<T> {
  let minuteur: ReturnType<typeof setTimeout> | undefined
  const attente = new Promise<T>((resolve) => {
...
export function atcMiddleware(appSlug: string, options: AtcMiddlewareOptions = {}) {
  ...
  return async function middleware(request: NextRequest): Promise<NextResponse> {
    ...
    const userData = await avecDelai(
    ...
    const sessionData = await avecDelai(
    ...
    if (claims.present) {
      ...
      const rpc = await avecDelai(
        supabase.rpc('has_app_access', { p_app: appSlug }).then((r) => r.data),
        null,
      )
      allowed = rpc === true
    }
