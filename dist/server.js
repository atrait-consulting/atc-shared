 ...
 */
export async function hasAppAccess(appSlug) {
    const supabase = await createAtcServerClient();
...
export async function appRole(appSlug) {
    const supabase = await createAtcServerClient();
 ...
 */
export async function requireApp(appSlug) {
    const user = await getAtcUser();
    if (!user)
        redirect(`${HUB.login}?app=${encodeURIComponent(appSlug)}`);
    if (!(await hasAppAccess(appSlug)))
        redirect(`${HUB.refus}?app=${appSlug}`);
}
/** URL de déconnexion : un formulaire POST, jamais un `signOut()` côté client. */
export const signOutUrl = () => HUB.signOut;
/** Mémorise l'application ouverte, pour que la racine y renvoie la prochaine fois. */
export async function touchLastApp(appSlug) {
    const supabase = await createAtcServerClient();
